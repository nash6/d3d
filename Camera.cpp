...
void Camera::walk(float units)
{
	...
	if (_cameraType == LANDOBJECT){
		judgeFollow(D3DXVECTOR3(_look.x, 0.0f, _look.z), units);
	}
...
void Camera::strafe(float units)
{
	// move only on xz plane for land object
	if (_cameraType == LANDOBJECT){
		judgeFollow(D3DXVECTOR3(_right.x, 0.0f, _right.z), units);
	}
...
void Camera::fly(float units)
{
	...
	if (_cameraType == LANDOBJECT){
		judgeFollow(D3DXVECTOR3(0, 1.0f, 0), units);
	}
...
bool Camera::getFollow() {
	return followBox;
}

void Camera::setBoxSize(float h, float w, float d) {
	boxH = h;
	boxW = w;
	boxD = d;
}
...
bool  Camera::floatAbsBigger(float a, float b) {
	if (abs(a) > abs(b))
		return true;
	else
		return false;
